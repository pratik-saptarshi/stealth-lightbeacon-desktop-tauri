...
renders the operator workflow, and keeps all Tauri command details behind
`src/lib/desktop.ts`.
...
terminal result retrieval, artifact retrieval, recon transport, and snapshot
persistence. The frontend does not talk to HTTP directly; Rust remains the
transport boundary.
